...
interface CommitmentsErrorProps {
  error: Error & { digest?: string };
  reset: () => void;
}

export default function CommitmentsError({ error, reset }: CommitmentsErrorProps) {
  useEffect(() => {
    console.error("Commitments page error:", error);
  }, [error]);
    ...
    <PageContainer>
      <div className="flex flex-col items-center justify-center min-h-[50vh] p-6 text-center font-sans">
        ...
        <p className="mt-2 text-xs text-muted-foreground max-w-md">
          An error occurred while trying to load the commitments tracker. Please try again.
        </p>
        {error.message && (
        ...
        <div className="mt-6">
          <Button
            type="button"
            onClick={reset}
            className="text-xs bg-brand hover:bg-brand/90 text-white flex items-center gap-1.5 h-9 cursor-pointer"
          >
