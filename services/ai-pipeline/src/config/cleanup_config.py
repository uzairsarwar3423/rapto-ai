...
CONTEXTUAL_FILLERS (informational, NOT in the unambiguous list):
  ...
  to Stage 2 (OpenAI), not this cheap rule-based pass. Design boundary
...
to Stage 2 (OpenAI), not this deterministic pass.
"""

# ─── Stage 2: Grammar Normalizer Batching ────────────────────────────────────

GRAMMAR_BATCH_MAX_TURNS: int = 18
"""Maximum number of merged turns per OpenAI batch.
...
"""Approximate max input tokens per OpenAI batch.
...
"""Maximum concurrent OpenAI batch calls from the cleanup stage.

This is the cleanup stage's LOCAL concurrency limit — it is nested inside
(and always ≤) Day 46's process-wide OpenAIClient semaphore ceiling
(openai_max_concurrent_calls in settings.py).
...
TOKEN_ESTIMATE_SAFETY_MARGIN: float = 1.05
"""Multiplier applied to local tiktoken counts to ensure we don't accidentally exceed OpenAI's actual budget."""
